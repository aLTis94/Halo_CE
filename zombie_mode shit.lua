...
--BIPEDS
	...
	BIPED_ANIMATION_STATE = {
		...
		[20]= "flood_vehicle walking",--airborne
		[21]= "flood_vehicle walking",  			--land_soft
		[22]= "flood_vehicle walking",  			--land_hard
		...
		[39]= "flood_vehicle leap_airborne", 	--leap start
...
--SHOP
	...
	zombie_kill_normal = 10-- How much money you get for killing a common zombie
	player_kill_penalty = 150-- How much money player loses for killing another player
...
--COMMANDS AND STUFF
	if(1) then --	This is only used so I could hide these
		...
		ai_deletion_distance = 20--	Distance that AI must be away from the player in order to get deleted (world units)
		player_death_zombie_spawn = true--	Should a zombie spawn where a player died?
		
		player_lives = 3--	After how many deaths player will become a zombie
		
		ai_position_update_rate = 4		--How often AI position updates in ticks. Default is 4, setting below 4 causes desync
		ai_animation_update_rate = 7	--How often AI animation states are updated in ticks. Setting this to low values
...
function OnScriptLoad()
	obj_table, same = {}, {}
	timer(1000, "get_objects")
	debug_person = 1
	execute_command("vehicle_incremental_rate "..ai_position_update_rate)
	object_table_ptr = sig_scan("8B0D????????8B513425FFFF00008D")
	register_callback(cb['EVENT_GAME_START'],"OnGameStart")
	if(lookup_tag("weap", dead_weapon) ~= 0 and flood_gametype == get_var(0, "$mode")) then
...
function OnPlayerSpawn(PlayerIndex)
	timer(100, "SwitchTeam", PlayerIndex)
...
function OnPlayerDeath(PlayerIndex, KilledIndex)
	...
	--check if he was in red team too
	if tonumber(get_var(PlayerIndex, "$deaths")) > player_lives - 1 then
...
function get_objects()
    ...
    for i = 0,object_count-1 do
        local object = read_dword(first_object + i * 0xC + 0x8)
        if object ~= 0 and object ~= 0xFFFFFFFF then
            -- (0 = Biped) (1 = Vehicle) (2 = Weapon) (3 = Equipment)
            -- (4 = Garbage) (5 = Projectile) (6 = Scenery) (7 = Machine)
            -- (8 = Control) (9 = Light Fixture) (10 = Placeholder) (11 = Sound Scenery)
            if read_word(object + 0xB4) == 0 then -- ^ = This
				local name = read_string(read_dword(read_word(object) * 32 + 0x40440038))
				if name == biped then
					for i = 0,#obj_table do
						same[object] = false
						if obj_table[i] == object then
							same[object] = true
							break
						end
					end
					if not same[object] then
						table.insert(obj_table, object)
						print("biped added to table: "..object)
					end
				end
            end
        end
...
function OnTick()
	...
	for ID, info in pairs (AI) do
		...
		if(AI[ID] ~= nil and ID ~= 0) then	--			Read biped info
			if(read_float(ID + 0xE0) < 0.01) then --			If biped is dead
				RemoveBiped(ID)
				say_all("biped died")
			else 
				...
				if(AI[ID] ~= nil) then
					AI[ID]["x"], AI[ID]["y"], AI[ID]["z"] = read_vector3d(ID + 0x5C)
					AI[ID]["x_vel"], AI[ID]["y_vel"], AI[ID]["z_vel"] = read_vector3d(ID + 0x68)
					AI[ID]["yaw"] = read_float(ID + 0x224)
					AI[ID]["pitch"] = read_float(ID + 0x228)
					
					GetDamagerID(ID)
					
					local unit_state = read_byte(ID + 0x2A3)
					...
					if(unit_animation == nil) then
						...
						RemoveBiped(ID)
						say_all("Biped had no vehicle for state "..unit_state)
					end
					...
					if AI[ID] ~= nil then
						...
						if(AI[ID]["vehicle"] == nil) then
							...
							if(vehicle_object ~= 0) then
								...
								if(unit_animation ~= nil and vehicle_name ~= tag_directory..unit_animation and second_count%ai_animation_update_rate == 0) then
									destroy_object(AI[ID]["vehicle"])
									SpawnVehicle(ID, unit_animation)
								end
								...
								write_vector3d(vehicle_object + 0x68, AI[ID]["x_vel"], AI[ID]["y_vel"], AI[ID]["z_vel"])
								...
								if(AI[ID]["time_alive"] == 100) then
									...
									write_float(ID + 0xE0, 1.0 + player_count*0.1)
		...
		--					If biped does not exist any more
			say_all("biped doesn't exist anymore :'v")
			RemoveBiped(ID)
		end
	end
...
function RemoveBiped(ID)--	Remove dead/non-existent biped and spawn dead weapon
	say_all("a biped was removed")
	--destroy_object(ID)
	write_float(ID + 0xE0, 0)
	if(AI[ID]["vehicle"] ~= nil) then
		destroy_object(AI[ID]["vehicle"])
	end
	if(ID ~= 0) then
