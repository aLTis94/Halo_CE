...
--	CONFIG
	...
	--	This script will only work on this game mode
	game_mode_name = "altis_ts_nr"
	
	--	Players need to be this close from each other in order to catch (in world units)
	catch_distance = 0.7
	
	--	Game will start after this delay (in seconds)
	game_start_delay = 8
	
	--	How many rounds until game over
	round_count = 3
	
	--	Round time (in seconds)
	round_time = 240
	
	--	How long seeker cannot do anything to let hiders hide (in seconds)
...
function CheckGameMode()
	if(game_mode_required == false) then
		return false
	end

	if(get_var(0, "$mode") == game_mode_name) then
		rounds = 0
		game_started = false
		game_over = false
		player_locations = {}
...
function StartRound()
	...
	if(player_present(last_caught)) then
		execute_command("st "..last_caught.." blue")
	else
		while (1) do
			local random_player = rand(1,16)
			if(player_present(random_player)) then
				execute_command("st "..random_player.." blue")
...
function OnPlayerDeath(PlayerIndex)
	if(game_started) then
		timer(33, "SwitchTeam", PlayerIndex, 1)
	end
end
...
function OnTick()
	...
	if(game_started == false) then
		if(tonumber(get_var(0, "$blues")) == 0) then
			while (1) do
				local random_player = rand(1,16)
				if(player_present(random_player)) then
					execute_command("st "..random_player)
					last_caught = random_player
		...
		for i=1,16 do
			if(player_alive(i) == true) then 
				...
				if(get_var(i, "$team") == "blue") then
					...
					write_vector3d(player_object + 0x5C, x, y, (z + i*3))
					write_vector3d(player_object + 0x68, 0, 0, 0)
					
					if(release_timer > 0 and console_messages) then
						if(player_country[i] == 1) then
				...
				elseif(release_timer > 0 and console_messages) then
					if(player_country[i] == 1) then
	...
	if(current_round_time == 0) then
		...
		for i = 1,16 do
			if(get_var(i, "$team") == "red") then
				execute_command("score "..i.." +1")
			end
		end
		game_started = false
		timer(100, "StartRound")
		return false
	end
	...
	for i=1,16 do
		if(player_alive(i) == true) then 
			local player_object = get_dynamic_player(i)
			if(player_object == 0) then
				say_all("Player has no object")--	Remove me later
				say_all("This would have crashed")--	Remove me later
				break
			end
			...
			if(player_team == "blue") then
				...
				for j=1,16 do
					if(player_alive(j) and get_var(j, "$team") == "red") then
						...
						if(other_player_object == 0) then
							say_all("Other player has no object")--	Remove me later
							say_all("This would have crashed")--	Remove me later
							break
						end
						local x2,y2,z2 = read_vector3d(other_player_object + 0x5C)
						if(x1 == nil or y1 == nil or z1 == nil or x2 == nil or y2 == nil or z2 == nil) then
							say_all("Wrong coordinates")--	Remove me later
							say_all("This would have crashed")--	Remove me later
							break
						end
