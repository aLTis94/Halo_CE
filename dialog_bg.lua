...
--CONFIG
	...
	local frag_name = "weapons\\frag grenade\\frag grenade"
	
	-- 					{1 = delay after this sound, chance of this sound playing}
	local DIALOG = {
		["scrn_plr_wrswpn"] = {45, 1},
		["chr_vcljmp"] = {55, 1},
		["chr_kllfoe"] = {55, 1},
		["chr_kllfoe_vclbmp"] = {90, 1},
		["dwn_wpn_snpr"] = {65, 1},
		["entervcl_drvr"] = {45, 1},
		["entervcl_gnr"] = {45, 1},
		["grt_intovcl_mine"] = {70, 1},
		["grt_plr_vcl"] = {65, 1},
		["grt"] = {25, 1},
		["scld_plr_hrt_blt"] = {60, 1},
		["scld_plr_vclcrazy"] = {70, 1},
		["strk_grnd"] = {50, 1},
		["strk_snpr"] = {45, 1},
		["thnk_plr_btrwpn"] = {55, 1},
		["pain_fall"] = {20, 2},
		["whn"] = {40, 1},
		["laugh"] = {50, 1.1},
	}
...
function Initialize()
	if get_var(0, "$map") == "bloodgulch" then
...
function PlaySound(i, sound)
	...
	if PLAYERS[i].last_dialog_timer == 0 and last_dialog_timer == 0 then
		...
		if math.random() < DIALOG[sound][2] then
			PLAYERS[i].last_dialog_timer = DIALOG[sound][1]
			last_dialog_timer = math.floor(DIALOG[sound][1]/2)
			
			for j=0,15 do
				if player_present(j) and (player_alive(j) == false or DistanceBetweenPlayers(i,j) < 19) then
					rprint(j, "|nvoice~"..PLAYERS[i].name.."~"..PLAYERS[i].gender.."_"..sound)
				end
			end
		end
	end
...
function OnPlayerDeath(i, causer)
	...
	elseif PLAYERS[i].weapon_name ~= nil and PLAYERS[i].weapon_name == sniper_name or PLAYERS[i].weapon_name == gauss_name then
...
function Woohoo()
	for i=1,16 do
		if player_present(i) and PLAYERS[i].dead == false and PLAYERS[i].vehicle ~= nil and PLAYERS[i].vehicle_seat ~= 0 and PLAYERS[i].vehicle_name ~= falcon_name then
			if PLAYERS[i].vehicle_airborne == 40 then
...
function OnDamage(i, causer, meta_id)
	...
	if tag ~= 0 then
		...
		if causer~=0 and PLAYERS[i].team == PLAYERS[causer].team and i~=causer and player_alive(i) and player_alive(causer) == true then
		...
		elseif name == "globals\\falling" then
			PlaySound(i, "pain_fall")
		elseif PLAYERS[i].shields < 0.1 and PLAYERS[i].health < 0.9 and PLAYERS[i].vehicle == nil then
			PlaySound(i, "whn")
		end
	end
end
...
function OnWeaponPickup(i)
	if PLAYERS[i].m_player ~= nil then
		local object = get_object_memory(read_dword(PLAYERS[i].m_player + 0x24))
		if object ~= 0 and read_word(object + 0xB4) == 2 then
			local unloaded_ammo = read_word(object + 0x2B6)
			local loaded_ammo = read_word(object + 0x2B8)
			if unloaded_ammo == 0 and loaded_ammo == 0 then
				PlaySound(i, "scrn_plr_wrswpn")
			else
				local name = GetName(object)
				if name == spartan_laser_name or name == rl_name or name == gauss_name then
