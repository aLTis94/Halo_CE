...
--	Need to test if item queue actually works
--	Several players need to buy items at once and check if correct items drop

--	Could try removing all zombie vehicles every second via map scripts instead of manually removing them

--	Could try removing all synced projectiles that cause damage effects (since that causes exceptions)
...
--VEHICLES
	...
	fuel_hud_type = 1
	-- At what distance can players refuel a vehicle (in world units)
	fuel_distance = 2
