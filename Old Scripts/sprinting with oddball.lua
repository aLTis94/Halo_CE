...
-- Sprinting gives player a specified weapon and changes its ammo depending on energy to sync enery meter on the hud
-- Player cannot sprint while holding specified weapons

--	COULD DO: 			prevent player from switching weapons
--	COULD DO:			check flag and oddball based on their MetaID from globals
...
-- Can't sprint with the following weapons
flag = "reach\\objects\\weapons\\multiplayer\\flag\\flag"
ball = "weapons\\ball\\ball"

-- Bipeds that are in the map. 2 is male and 1 is female
BIPEDS = 
{
...
sprint_remove_check = 	300	--	(in ms) 	How often to check if sprint weapon needs to be removed
spawned_object_check =	33	--	(in ms) 	After an object was spawned, how often to check if the object is sprint weapon
sprint_delay =			5	--	(in ticks)	How often to check if player is sprinting (lower means more often)
...
function SprintObjectRemove(ObjectID)--	Check if players aren't holding the sprint weapon anymore. If they are, call this function again until it is dropped
...
function CheckSprint(PlayerIndex)
    ...
    if(tonumber(held_obj_id) ~= 4294967295) then
		...
		name = read_string(read_dword(read_word(held_weapon) * 32 + 0x40440038))
    ...
    if(runningforward and moving[PlayerIndex] == false and invehicle == false and weapon_name ~=flag and weapon_name ~= ball and current_biped[PlayerIndex] ~= 0) then -- Player just started moving
    ...
    elseif((runningforward == false and moving[PlayerIndex] == true) or invehicle == true or weapon_name == flag or weapon_name == ball) then -- Player stopped moving
		moving[PlayerIndex] = false
...
function DropWeapon(PlayerIndex)--	This is used to prevent players from picking up a third weapon while sprinting
	...
	if(name ~= sprint_weap and name ~=female_weap and name ~= flag and name ~= oddball) then
...
function OnRest(PlayerIndex)
	...
	if(tonumber(held_obj_id) ~= 4294967295) then--	Get the weapon that player is holding
		...
		local name = read_string(read_dword(read_word(held_weapon) * 32 + 0x40440038))
		weapon_name = string.format("%s", name)
	else
		weapon_name = nil
	end
...
function OnSprint(PlayerIndex)
	...
	if(weapon_name ~= sprint_weap and weapon_name ~= female_weap and weapon_name ~= flag and weapon_name ~= ball and delay == 0) then
		delay = sprint_delay
		for j=1,4 do
			metaid = (read_dword(player + 0x2F8 + (j - 1) * 4))
			if(metaid ~= 0xFFFFFFFF) then
...
function OnTick()
    ...
    for PlayerIndex = 1,16 do
        if(player_alive(PlayerIndex) == true) then
			...
			if(biped ~= 0) then
				name = read_string(read_dword(read_word(biped) * 32 + 0x40440038))
			...
			if(tonumber(get_var(PlayerIndex,"$lvl")) >= admin_level) then
                ...
                if(sprinting[PlayerIndex] == true) then
                    if(player_energy[PlayerIndex] > 0) then
						...
						execute_command("ammo \""..PlayerIndex.."\" \""..ammo.."\" 3")
						execute_command("ammo \""..PlayerIndex.."\" \""..ammo.."\" 4")
