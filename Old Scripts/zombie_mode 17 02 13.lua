...
--BIPEDS
	tag_directory= "ai\\floodcombat_human\\"	--where all of your biped tags are.
	biped        = tag_directory.."biped" 	--the biped you want to sync
	dead_weapon  = tag_directory.."dead" 		--weapon that will be used to fake a dead biped
...
--COMMANDS AND STUFF
	if(1) then --	This is only used so I could hide these
		...
		player_death_zombie_spawn = false--	Should a zombie spawn where a player died?
...
function InitializeVehicles()
	for key,value in pairs (BIPED_ANIMATION_STATE) do
		local vehicle_tag = lookup_tag("vehi", tag_directory.."veh\\"..value)
			...
			if(read_float(biped_object + 0xE0) < 0.01) then --			If biped is dead
				...
				if(AI[ID] ~= nil) then
					...
					if(unit_animation == nil) then
						...
						say_all("Biped had no vehicle for this animation. Yell at altis!")
					end
					...
					if(AI[ID]["vehicle"] == nil) then
						...
						if(vehicle_object ~= 0) then
							...
							if(unit_animation ~= nil and vehicle_name ~= tag_directory.."veh\\"..unit_animation and second_count%ai_animation_update_rate == 0) then
								destroy_object(AI[ID]["vehicle"])
								SpawnVehicle(ID, unit_animation)
							end
...
function SpawnVehicle(ID, unit_animation)
	AI[ID]["vehicle"] = spawn_object("vehi", tag_directory.."veh\\"..unit_animation, AI[ID]["x"], AI[ID]["y"], AI[ID]["z"], GetYaw(ID))
end
...
function CheckBiped(ID, name, object)
	if(name == biped and AI[ID] == nil) then
