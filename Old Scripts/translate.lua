...
--TODO:
-- fix toggle translation

-- CONFIG

	translation_type = "Google" -- choose Google or Yandex (Yandex is quite broken and needs a key)

	minimum_length = 4 -- if message is this short, we don't need to translate it

	DEFAULT_MSG_PREFIX = "**SAPP** "

	geolocation_timeout = 10 * 1000 -- how much to wait for geolocation to answer before giving up
...
function OnJoin(PlayerIndex)
	timer(2000, "say", PlayerIndex, "The server is using "..translation_type.." translate")
...
function OnCommand(PlayerIndex, Command)
	if(PlayerIndex > 0) then
		...
		if(Command == COMMAND_TO_SET_LANG_TO_ENGLISH) then
			...
			say_all(get_var(PlayerIndex, "$name").." switched to English")
		...
		if(Command == COMMAND_TO_SET_LANG_TO_SPANISH) then
			...
			say_all(get_var(PlayerIndex, "$name").." switched to Spanish")
...
function OnChat(PlayerIndex, Message, message_type)
	...
	if(first_character ~= "\\" and first_character ~= "/" and string.len(Message) > minimum_length) then
		...
		for i=1,16 do
			if(player_present(i)) and player_language[i] == from_language then
				if message_type == 0 then
					say(i, player_name..": "..Message)
				elseif player_team == get_var(i, "$team") then
					say(i, "["..player_name.."]: "..Message)
				else
		...
		local random_string = string.random(64)
		if string.lower(translation_type) == "google" then
			...
			url = URL .. from_language .. "-" .. to_language .. URL2 .. URL_Encode(Message)
			async_table[random_string] = http_client.http_get(url, true)
...
function CheckLocation(ResultName, PlayerIndex, PlayerIP)
	...
	geolocation_timers[PlayerIndex] = geolocation_timers[PlayerIndex] + 1
	if geolocation_timers[PlayerIndex] > geolocation_timeout or player_present(PlayerIndex) == false then
		geolocation_timers[PlayerIndex] = 0
		http_client.http_destroy_response(async_table[ResultName])
		say(PlayerIndex, "escribe /esp para elegir el idioma espanol")
        return
	end
    ...
    if http_client.http_response_received(async_table[ResultName]) then
		...
		if player_language[PlayerIndex] == "es" then
			say(PlayerIndex, "Write /eng if you speak English")
		else
			say(PlayerIndex, "escribe /esp para elegir el idioma espanol")
		end
		...
		timer(10000, "TellLanguage", PlayerIndex)
...
function TellLanguage(PlayerIndex)
	PlayerIndex = tonumber(PlayerIndex)
	if player_language[PlayerIndex] == "es" then
		say_all(get_var(PlayerIndex, "$name").." speaks Spanish")
	else
		say_all(get_var(PlayerIndex, "$name").." speaks English")
	end
end
...
function CheckResult(ResultName, PlayerName, Language, OriginalMessage, MessageType, PlayerTeam, VehicleObjectID)
    if http_client.http_response_received(async_table[ResultName]) then
        if http_client.http_response_is_null(async_table[ResultName]) then
            SendMessage(PlayerName, Language, OriginalMessage, OriginalMessage, tonumber(MessageType), PlayerTeam, tonumber(VehicleObjectID))
        else
			local response = string.gsub(string.gsub(ffi.string(http_client.http_read_response(async_table[ResultName])),"\t"," "),"\n","")
			local code = string.sub(response, 9, 12)
			if(code == "200,") then
				response = string.sub(response, string.find(response, "text") + 8, response:len() - 3)
				SendMessage(PlayerName, Language, OriginalMessage, response, tonumber(MessageType), PlayerTeam, tonumber(VehicleObjectID))
			else
				SendMessage(PlayerName, Language, OriginalMessage, OriginalMessage, tonumber(MessageType), PlayerTeam, tonumber(VehicleObjectID))
			end
        end
...
function SendMessage(PlayerName, Language, OriginalMessage, TranslatedMessage, MessageType, PlayerTeam, VehicleObjectID)
	...
	elseif(MessageType == 1) then
		...
		for i=1,16 do
			if player_present(i) and player_language[i] ~= Language then
	...
	elseif(MessageType == 2) then
		...
		for i=1,16 do
			if player_alive(i) and player_language[i] ~= Language then
				local player = get_dynamic_player(i)
				if(player ~= 0) then
					local vehicle_objectid = read_dword(player + 0x11C)
					if(VehicleObjectID == vehicle_objectid) then
